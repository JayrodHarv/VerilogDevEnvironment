...
module alu (
  ...
  // adder
  always @(rsa, opb, imm_ext, c_in, funct, alu_op) begin
    add_out = 33'h000000000;
    case (alu_op[3:1])
      3'b000, 3'b001: begin
        if (funct == ADD) add_out = rsa + opb;
        else if (funct == SUB) add_out = rsa - opb;
        else if (funct == ADC) add_out = rsa + opb + c_in;
        ca = add_out[32];
      end
      3'b010: begin
        add_out = rsa + imm_ext;
  ...
  // output mux
  always @(add_out, log_out, shf_out, funct, alu_op) begin
    if ((alu_op[3:1] != 3'b000) && (alu_op[3:1] != 3'b001)) begin
  ...
  always @(posedge clk) alu_result <= alu_out;
